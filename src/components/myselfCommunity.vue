    ...
    if (storedToken && storedUsername) {
      try {
        const response = await fetch(`${baseURL}/api/user/verify/`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${storedToken as string}`,
            'Content-Type': 'application/json'
          },
          credentials: 'omit', // 不发送凭证，避免触发登录重定向
          redirect: 'manual' // 手动处理重定向
      ...
      localStorage.removeItem('username');
        ...
        console.error('验证用户状态失败（可能是网络问题），保留登录状态:', error);
        // 网络错误时不再清除登录状态，避免影响用户体验
        // 仍然确保store中的状态与localStorage同步
        userStore.setUsername(storedUsername);
      }
    }
  };
  
  // 可选：可以注释掉这行，完全移除token验证，让登录状态只由App.vue和stores.ts管理
  // verifyToken();
});
